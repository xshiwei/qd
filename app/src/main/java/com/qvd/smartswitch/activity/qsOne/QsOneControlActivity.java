...
public class QsOneControlActivity extends BaseActivity {
    ...
    @BindView(R.id.iv_device_control_state)
    ImageView ivDeviceControlState;
    @BindView(R.id.tv_device_control_state)
    TextView tvDeviceControlState;

    private static String TAG = QsOneControlActivity.class.getSimpleName();
    ...
    @OnClick({R.id.iv_device_control_goback, R.id.iv_device_control_more, R.id.iv_device_control_state, R.id.iv_sound})
    public void onViewClicked(View view) {
        switch (view.getId()) {
            ...
            case R.id.iv_device_control_more:
                startActivity(new Intent(this, DeviceSettingActivity.class).putExtra("bledevice", bledevice));
                overridePendingTransition(R.anim.push_left_in, R.anim.push_left_out);
                break;
            case R.id.iv_device_control_state:
                if (isState) {
                    isState = false;
                    dialog.show();
                    //关灯
                    writeToBle(String.valueOf("FE010000FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF"));
                } else {
                    isState = true;
                    dialog.show();
                    //开灯
                    writeToBle(String.valueOf("FE01000100FFFFFFFFFFFFFFFFFFFFFFFFFFFFFF"));
                }
                break;
            case R.id.iv_sound:
                //语音控制开关灯
    ...
    private void writeToBle(String s) {
        ...
        BleManager.getInstance().write(bledevice, "0000fff0-0000-1000-8000-00805f9b34fb", "0000fff6-0000-1000-8000-00805f9b34fb", CommandUtils.decodeToByteArray(s), true, new BleWriteCallback() {
            @Override
            public void onWriteSuccess(int current, int total, byte[] justWrite) {
                ...
                if (!isState) {
                    ivDeviceControlState.setImageResource(R.mipmap.device_control_turnoff);
                    tvDeviceControlState.setText(R.string.device_control_off);
                    ...
                    DeviceDaoOpe.insertData(QsOneControlActivity.this, deviceLogVo);
                    dialog.dismiss();
                } else {
                    ivDeviceControlState.setImageResource(R.mipmap.device_control_turnon);
                    tvDeviceControlState.setText(R.string.device_control_on);
    ...
    private RecognizerListener mRecognizerListener = new RecognizerListener() {
        ...
        public void onVolumeChanged(int volume, byte[] data) {
            if (volume < 10) {
                Picasso.with(QsOneControlActivity.this).load(R.mipmap.voice_empty).into(ivSound);
            } else {
                Picasso.with(QsOneControlActivity.this).load(R.mipmap.voice_full).into(ivSound);
            }
        }
